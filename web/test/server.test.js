...
test("heartbeat, listing and command round-trip", async () => {
  ...
  try {
    let response = await fetch(`${base}/api/v1/heartbeat`, { method: "POST", headers, body: JSON.stringify({ turtleId: 12, data: { jobId: "job-1", turtleName: "Miner", width: 16, length: 16, inventory: [{ slot: 1, name: "minecraft:diamond", count: 7 }] } }) });
    assert.equal(response.status, 202);
    response = await fetch(`${base}/api/v1/turtles`, { headers });
    const listed = (await response.json())[0]; assert.equal(listed.turtleName, "Miner"); assert.equal(listed.inventory[0].name, "minecraft:diamond");
