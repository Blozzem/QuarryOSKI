...
function statisticsOf(turtle){const stats=turtle.statistics&&typeof turtle.statistics==="object"?turtle.statistics:{};return {blocks:Number(stats.blocks??turtle.blocks)||0,surfaceMoves:Number(stats.surfaceMoves)||0,verticalMoves:Number(stats.verticalMoves)||0,services:Number(stats.services)||0,fluids:Number(stats.fluids??turtle.fluids)||0,seals:Number(stats.seals??turtle.seals)||0,blockTypes:stats.blockTypes&&typeof stats.blockTypes==="object"?stats.blockTypes:{},oreTypes:stats.oreTypes&&typeof stats.oreTypes==="object"?stats.oreTypes:{}};}
function ranked(values){return Object.entries(values).map(([name,count])=>({name,count:Number(count)||0})).filter(row=>row.count>0).sort((left,right)=>right.count-left.count||left.name.localeCompare(right.name));}
function renderRanking(root,values,limit){root.replaceChildren();const rows=ranked(values).slice(0,limit);if(!rows.length){const empty=document.createElement("div");empty.className="ranking-empty";empty.textContent="Noch keine Daten";root.append(empty);return;}const list=document.createElement("div");list.className="ranking-list";for(const row of rows){const line=document.createElement("div"),label=document.createElement("span"),count=document.createElement("b");line.className="ranking-row";label.textContent=itemLabel(row.name);label.title=row.name;count.textContent=number(row.count);line.append(label,count);list.append(line);}root.append(list);}
...
function renderDetail(turtle){
  ...
  const stats=statisticsOf(turtle),oreRows=ranked(stats.oreTypes),blockRows=ranked(stats.blockTypes);put(node,".statistics-meta",turtle.phase==="complete"?"Abgeschlossen":"Live");put(node,".stats-blocks",number(stats.blocks));put(node,".stats-ores",number(oreRows.reduce((sum,row)=>sum+row.count,0)));put(node,".stats-types",number(blockRows.length));put(node,".stats-surface",number(stats.surfaceMoves));put(node,".stats-vertical",number(stats.verticalMoves));put(node,".stats-services",number(stats.services));put(node,".stats-fluids",number(stats.fluids));put(node,".stats-seals",number(stats.seals));
  root.append(node);drawMap(node.querySelector(".quarry-map"),turtle);renderInventory(node.querySelector(".inventory"),turtle);renderRanking(node.querySelector(".ore-ranking"),stats.oreTypes,10);renderRanking(node.querySelector(".block-ranking"),stats.blockTypes,10);
